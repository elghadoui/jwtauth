...
export const ClientBarChart = ({ data, loading }) => {
    if (loading) {
        return (
            <div className="h-80 flex items-center justify-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
    ...
    const chartData = data.slice(0, 10).map(item => ({
        name: (item.client || item.rsclient || 'Inconnu').substring(0, 20),
        palettes: item.totalPalettes || 0,
        poids: (item.totalPoids || 0) / 1000 // Convertir en tonnes
    }));
...
export const StationPieChart = ({ data, loading }) => {
    if (loading) {
        return (
            <div className="h-80 flex items-center justify-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
    ...
    return (
        <ResponsiveContainer width="100%" height={320}>
            <PieChart>
                ...
                <Tooltip
                    formatter={(value) => `${(value / 1000).toFixed(1)} tonnes`}
                    contentStyle={{
                        backgroundColor: '#FFF',
                        border: '1px solid #E5E7EB',
                        borderRadius: '8px',
...
export default { TimelineChart, CountryPieChart, ProductBarChart, StationBarChart, ClientBarChart, StationPieChart };
